...
int free_page_number = TOTAL_PAGE_NUMBER;
...
page_t* get_back_redundant_memory(page_t* alloc_page, int get_page_level, int req_page_level){
    for(int r = get_page_level-1 ; r >= req_page_level; r--){
        ...
        //printf("****** cutoff memory from %d to %d (totally %d page) in order %d list *******\r\n",next_buddy_start_page->page_index, next_buddy_end_page->page_index, next_buddy_end_page->page_index - next_buddy_start_page->page_index+1, r);
...
page_t* get_pages_from_list(int order){
    ...
    for(i = order; i < MAX_ORDER; i++){
        if(!list_empty(&page_buddy[i])){
            alloc_page = list_entry(page_buddy[i].next, page_t, list);

            list_ptr_t* next_buddy =  (&(BUDDY_END(alloc_page, i)->list))->next;
            //printf("-----------------\n");
            ...
            next_buddy->prev = (&page_buddy[i]);
            page_buddy[i].next = next_buddy;
    ...
    if(i != order){
        alloc_page = get_back_redundant_memory(alloc_page, i, order);
    }
...
void free_page(unsigned long physical_addr){ 
	unsigned long page_frame_number =  (physical_addr - LOW_PAGE_POOL_MEMORY) >> 12;
...
unsigned long give_slab(int size){
    ...
    //intf("slub index is %d\n",slub_index);
    uart_puts("slub index is ");
	uart_put_int(slub_index);
	uart_puts("\n");
...
void split_page_to_slub(page_t* page, int slub_index){
    ...
    for(int i = 0; i < slub_num -1; i++){
        ...
        //intf("________________________________\n");
        //intf("split page addr is:%x\n", tmp);
...
void init_kmalloc_caches(){
    for(int s = 0; s < SLUB_NUMBER; s++){
        ...
        INIT_LIST_HEAD(&kmem_cache_arr[s].cache_cpu.partial);
    }
}
...
unsigned long give_one_slub(int slub_index){
    ...
    //intf("slub want to allocate slub addr is: %x\n", slub);
...
page_t* find_partial_free_slub(int slub_index){
    ...
    if(!list_empty(&kmem_cache_arr[slub_index].cache_cpu.partial)){
        //intf("xxxxxxxxxxxxxxxxpartial list not empty\n");
        ...
        do{			
            ...
            //intf("partial page number is %d\n", page->page_index);
            //intf("partial page order is %d\n", page->order);
            //intf("partial page slub index is %d\n", page->slub_index);
            //intf("partial page slub num is %d\n", page->slub_num);
            //intf("-----\n");
            uart_put_int(page->slub_num);
            if(page->slub_num >= 1){
                //intf("partial has a free slub!!!!!!!!!\n");
                return page;
            }
            tmp = tmp->next;
    ...
    //intf("xxxxxxxxxxxxxxxxpartial has no free slub\n");
    return (page_t*)NULL;
}
...
void print_buddy_info(){
    ...
    for(int i = 0; i < MAX_ORDER; i++){
		int count = 0;
        if(!list_empty(&page_buddy[i])){
            list_ptr_t *tmp = page_buddy[i].next;
            ...
            while(1){
                ...
                page_t* page_next = list_entry((&(BUDDY_END(page, i))->list)->next, page_t ,list);
				count++;
                if((&(BUDDY_END(page, i))->list)->next == (&page_buddy[i])){
                    break;
                }
                page = page_next;
            }
...
void print_slub_info(){
    for(int i = 0 ; i < SLUB_NUMBER; i++){
        ...
        while(init_ptr != NULL){
			uart_puts("now addr is: ");
	    	uart_put_hex((unsigned long)init_ptr);
			uart_puts("\n");
...
void init_page_sys(){
	...
	//uart_puts("HIGH_PAGE_POOL_MEMORY is:  ");
	//uart_put_hex(HIGH_PAGE_POOL_MEMORY);
	//uart_puts("\n");
    ...
    for(int p = 0; p < TOTAL_PAGE_NUMBER; p++){
		...
		//uart_put_int(p);
		//uart_puts("].order = ");
		//uart_put_int(page_t_pool[p].order);
		//uart_puts("\n");
    }
	init_kmalloc_caches();
	print_buddy_info();
    unsigned long c1;
    c1 = allocate_memory(4096);
