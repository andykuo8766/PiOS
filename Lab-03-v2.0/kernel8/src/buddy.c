#include "buddy.h"

struct mem_block {
    struct mem_zone     zone;
    struct page        *pages;
};

struct mem_block global_mem_block;



void buddy_system_init(struct mem_zone *zone,struct page *start_page,unsigned long start_addr,unsigned long page_num){
    unsigned long i;
    struct page *page;
    struct free_area *area;
    // init memory zone
    zone->page_num = page_num;
    zone->page_size = BUDDY_PAGE_SIZE;
    zone->first_page = start_page;
    zone->start_addr = start_addr;
    zone->end_addr = start_addr + (page_num * BUDDY_PAGE_SIZE);
    // init each area
    for (i = 0; i < BUDDY_MAX_ORDER; i++){
        area = zone->free_area + i;
        INIT_LIST_HEAD(&area->free_list);
        area->nr_free = 0;
    }
    memset(start_page, 0, page_num * sizeof(struct page));
    // init and free each page
    for (i = 0; i < page_num; i++){
        page = zone->first_page + i;
        INIT_LIST_HEAD(&page->lru);
        buddy_free_pages(zone, page);
    }
}
					   
	
        
int page_is_buddy(struct page * page, unsigned long order) {
        return (PageBuddy(page) && (page->order == order));	
}

void buddy_free_pages(struct mem_zone *zone,struct page *page){
    unsigned long order = compound_order(page);
    unsigned long buddy_idx = 0, combinded_idx = 0;
    unsigned long page_idx = page - zone->first_page;
    while (order < BUDDY_MAX_ORDER-1){
        struct page *buddy;
        // find and delete buddy to combine
        buddy_idx = __find_buddy_index(page_idx, order);
        buddy = page + (buddy_idx - page_idx);
        if (!page_is_buddy(buddy, order))
            break;
        list_del(&buddy->lru);
        zone->free_area[order].nr_free--;
        // remove buddy's flag and order
        rmv_page_order_buddy(buddy);
        // update page and page_idx after combined
        combinded_idx = __find_combined_index(page_idx, order);
        page = page + (combinded_idx - page_idx);
        page_idx = combinded_idx;
        order++;
    }
    set_page_order_buddy(page, order);
    list_add(&page->lru, &zone->free_area[order].free_list);
    zone->free_area[order].nr_free++;
    //TODO: unlock zone->lock
}

					  
void dump_print(struct mem_zone *zone){
    unsigned long i;
    uart_puts("order(npage)\tnr_free\n");
    for (i = 0; i < BUDDY_MAX_ORDER; i++){
        uart_put_int(i);
		uart_puts("(");
        uart_put_int(1<<i);
		uart_puts(")\t\t");
        uart_put_int(zone->free_area[i].nr_free);
		uart_puts("\n");
    }
}

void BUDDY_BUG(char *f, int line){
	uart_puts("BUDDY_BUG in ");
	uart_puts(f);
	uart_puts(", ");
	uart_put_int(line);
	uart_puts(".\n");
}

void prepare_compound_pages(struct page *page, unsigned long order){
    unsigned long i;
    unsigned long nr_pages = (1 <<order);
    set_compound_order(page, order);
    __SetPageHead(page); 
    for(i = 1; i < nr_pages; i++){
        struct page *p = page + i;
        __SetPageTail(p); 
        p->first_page = page;
    }
}


void expand(struct mem_zone *zone, struct page *page,unsigned long low_order, unsigned long high_order,struct free_area *area){
    unsigned long size = (1 << high_order);
    while (high_order > low_order){
        area--;
        high_order--;
        size >>= 1;
        list_add(&page[size].lru, &area->free_list);
        area->nr_free++;
        // set page order
        set_page_order_buddy(&page[size], high_order);
    }
}

struct page *__alloc_page(unsigned long order,struct mem_zone *zone){
    struct page *page;
    struct free_area *area;
    unsigned long current_order = 0;
    for (current_order = order;current_order < BUDDY_MAX_ORDER; current_order++){
        area = zone->free_area + current_order;
        if (list_empty(&area->free_list)) {
            continue;
        }
        // remove closest size page
        page = list_entry(area->free_list.next, struct page, lru);
        list_del(&page->lru);
        rmv_page_order_buddy(page);
        area->nr_free--;
        // expand to lower order
        expand(zone, page, order, current_order, area);
        // compound page
        if (order > 0)
            prepare_compound_pages(page, order);
        else // single page
            page->order = 0;
        return page;
    }
    return 0;
}



struct page *buddy_get_pages(struct mem_zone *zone,unsigned long order){
    struct page *page;

    if (order >= BUDDY_MAX_ORDER){
		uart_puts("order is out of range\n");
        //BUDDY_BUG(__FILE__, __LINE__);
        return 0;
    }
    page = __alloc_page(order, zone);
    return page;
}

unsigned long buddy_num_free_page(struct mem_zone *zone){
    unsigned long i, ret;
    for (i = 0, ret = 0; i < BUDDY_MAX_ORDER; i++){
        ret += zone->free_area[i].nr_free * (1 <<i);
    }
    return ret;
}

void buddy_system(){
    
    struct mem_zone *zone;
	uart_puts("start init\n");
	buddy_system_init(&global_mem_block.zone,global_mem_block.pages,__buddy_start,MAX_PAGE);
	zone = &global_mem_block.zone;
    uart_puts("init done\n");
	
	struct page *p;
    buddy_get_pages(zone, 8);
	
    p = buddy_get_pages(zone, 8);
	
	dump_print(zone);
	uart_puts("num free: ");
	uart_put_int(buddy_num_free_page(zone));
    uart_puts("\n");

    if (p) buddy_free_pages(zone, p);


	dump_print(zone);
	uart_puts("num free: ");
	uart_put_int(buddy_num_free_page(zone));
    uart_puts("\n");	



}

//////////////////////////////////////////////////////////////
void __SetPageHead(struct page *page){
    page->flags |= (1 <<PG_head);
}
void __SetPageTail(struct page *page){
    page->flags |= (1 <<PG_tail);
}
void __SetPageBuddy(struct page *page){
    page->flags |= (1 <<PG_buddy);
}
//////////////////////////////////////////////////////////////
void __ClearPageHead(struct page *page){
    page->flags &= ~(1 <<PG_head);
}
void __ClearPageTail(struct page *page){
    page->flags &= ~(1 <<PG_tail);
}
void __ClearPageBuddy(struct page *page){
    page->flags &= ~(1UL<<PG_buddy);
}	
//////////////////////////////////////////////////////////////
int PageHead(struct page *page){
    return (page->flags & (1 <<PG_head));
}
int PageTail(struct page *page){
    return (page->flags & (1 <<PG_tail));
}
int PageBuddy(struct page *page){
    return (page->flags & (1 <<PG_buddy));
}
//////////////////////////////////////////////////////////////
void set_page_order_buddy(struct page *page, unsigned long order){
    page->order = order;
    __SetPageBuddy(page);
}
void rmv_page_order_buddy(struct page *page){
    page->order = 0;
    __ClearPageBuddy(page);
}
//////////////////////////////////////////////////////////////
unsigned long __find_buddy_index(unsigned long page_idx, unsigned int order){
    return (page_idx ^ (1 << order));
}

unsigned long __find_combined_index(unsigned long page_idx, unsigned int order){
    return (page_idx & ~(1 << order));
}
//////////////////////////////////////////////////////////////
unsigned long compound_order(struct page *page){
    if (!PageHead(page))
        return 0; //等珜
    //return (unsigned long)page[1].lru.prev;
    return page->order;
}
void set_compound_order(struct page *page, unsigned long order){
    //page[1].lru.prev = (void *)order;
    page->order = order;
}

