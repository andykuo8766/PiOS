        ...
        struct cpio_header **next)
{
    ...
    *filename = ((char *)archive) + sizeof(struct cpio_header);
	...
	if(memcmp(*filename, CPIO_FOOTER_MAGIC, sizeof(CPIO_FOOTER_MAGIC)) != 0){
		uart_hex(oct2bin(archive->c_mode,6)); 
		uart_puts("\t\t");
		uart_hex(filesize);						// file size in hex
		uart_puts("\t\t");
		uart_puts(*filename);		// filename
		uart_puts("\n");
	}
...
void cpio_ls(void *archive, unsigned long buf_len) {
    const char *current_filename;
    struct cpio_header *header, *next;
    void *result;
    int error;
    ...
    header = archive;
	uart_puts("c_mode\t\tSize\t\tFilename\n");
    for (i = 0; i < buf_len; i++) {
